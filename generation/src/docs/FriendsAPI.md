...
> Success Boop(ctx, userId).BoopRequest(boopRequest).Execute()
...
func main() {
	...
	boopRequest := *openapiclient.NewBoopRequest() // BoopRequest | 

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.FriendsAPI.Boop(context.Background(), userId).BoopRequest(boopRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `FriendsAPI.Boop``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `Boop`: Success
	fmt.Fprintf(os.Stdout, "Response from `FriendsAPI.Boop`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiBoopRequest struct via the builder pattern
...
> []LimitedUserFriend GetFriends(ctx).Offset(offset).N(n).Offline(offline).Execute()
...
func main() {
	...
	// response from `GetFriends`: []LimitedUserFriend
...
[**[]LimitedUserFriend**](LimitedUserFriend.md)
